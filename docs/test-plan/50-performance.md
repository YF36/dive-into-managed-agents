...
- 同一 endpoint 在 direct mode 和 aws-platform mode 的延迟差异在合理范围(预期 AWS 路径多 1 跳 < 50ms)
...
- `src/utils/timing.ts` 提供 `measureLatency(fn, n=100)` helper,返回 p50/p95/p99/mean/std
- 每个 case 跑 100 次取分布,warm-up 5 次丢弃
- 同 case 在 direct + aws-platform 两 mode 各跑一遍,记录对比
...
- `tests-cma/perf-results/<YYYY-MM-DD>-<mode>.json`(direct / aws-platform 分文件)
