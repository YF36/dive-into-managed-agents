...
> 产出物归属(findings / artifacts / decisions)说明见 sibling notes repo;本机维护者跨 repo workflow 详见 repo 根的 `INTERNAL.md`(gitignored)。
...
│   └── cross-vendor/         Phase 5(跨 vendor 设计假设对照测试)
└── scripts/
...
测试跑完产出的 raw 数据(`events.jsonl` / `http.jsonl` / `marks.json` / `metadata.json` / `case.md`)默认落到 sibling notes repo(具体路径配在 `.local-config.json` 的 `siblingArtifactRoot`,gitignored)。fallback 落到 `tests-cma/artifacts/`(gitignored,兜底)。

需要提炼成 finding / 升级 ADR 时,在 sibling notes repo 维护(具体路径 + workflow 详见 `INTERNAL.md`,本机维护者参考)。
